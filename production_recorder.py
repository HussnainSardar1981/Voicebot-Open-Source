...
class ProductionCallRecorder:
    ...
    def get_user_input_with_mixmonitor(self, timeout=10):
        ...
        try:
            # Start MixMonitor - records call audio stream
            # W option: stop recording when call ends
            # b option: only record audio going TO the channel (user speech)
            mixmonitor_cmd = f'EXEC MixMonitor {record_file}.wav,b'
            ...
            if os.path.exists(wav_file):
                ...
                if file_size > 800:  # Sufficient audio for transcription
                    # Transcribe with ASR
    ...
    def record_with_voice_interrupt(self, filename, timeout=3):
        ...
        try:
            # Start MixMonitor for interrupt detection
            mixmonitor_cmd = f'EXEC MixMonitor {record_file}.wav,b'
            result = self.agi.command(mixmonitor_cmd)
